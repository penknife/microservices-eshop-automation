...
try
{
    ...
    builder.Services.AddDbContext<NotificationsDbContext>(opts =>
        opts.UseNpgsql(connectionString, npgsql => npgsql.MigrationsHistoryTable("__ef_migrations_history", "notifications")));

    builder.Services.AddKafkaMessaging(builder.Configuration);
    builder.Services.AddKafkaConsumer<PaymentProcessedIntegrationEvent, PaymentProcessedHandler>(
    ...
    app.MapGet("/", () => Results.Ok(new { service = "notification-service", status = "ok" }));
